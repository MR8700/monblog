<?php $__env->startSection('title', 'Demandes de Services - Admin'); ?>
...
<section class="max-w-7xl mx-auto px-6 py-12" x-data="{ showTypeManager: false }">
    ...
    <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-soft mb-8">
        <form action="<?php echo e(url()->current()); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="md:col-span-2 relative">
                ...
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Client, Email, Service..." class="w-full pl-12 pr-6 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-medium">
            </div>
            <div>
                <select name="status" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-bold text-slate-700">
                    ...
                    <option value="quoted" <?php echo e(request('status') == 'quoted' ? 'selected' : ''); ?>>Devis envoyé</option>
            ...
            <div class="flex gap-2">
                <select name="type" class="flex-1 px-6 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-bold text-slate-700">
                    ...
                    <?php $__currentLoopData = $serviceTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($st->name); ?>" <?php echo e(request('type') == $st->name ? 'selected' : ''); ?>><?php echo e($st->name); ?></option>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-soft">
            ...
            <p class="text-3xl font-black text-slate-900"><?php echo e(number_format($requests->total())); ?></p>
        </div>
        <div class="bg-amber-50 p-8 rounded-[2.5rem] border border-amber-100/50 shadow-soft">
            ...
            <p class="text-3xl font-black text-amber-600"><?php echo e($requests->where('status', 'pending')->count()); ?></p>
        </div>
        <div class="bg-blue-50 p-8 rounded-[2.5rem] border border-blue-100/50 shadow-soft">
            ...
            <p class="text-3xl font-black text-blue-600"><?php echo e($requests->where('status', 'processing')->count()); ?></p>
        </div>
        <div class="bg-green-50 p-8 rounded-[2.5rem] border border-green-100/50 shadow-soft">
            ...
            <p class="text-3xl font-black text-green-600"><?php echo e($requests->where('status', 'delivered')->count()); ?></p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-slate-50">
                    <?php $__empty_1 = true; $__currentLoopData = $requests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $request): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-8 py-6">
                                <div>
                                    <p class="font-bold text-slate-900"><?php echo e($request->client_name); ?></p>
                                    <p class="text-xs text-slate-400"><?php echo e($request->client_email); ?></p>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-[10px] font-black uppercase tracking-widest">
                                    <?php echo e($request->service_type); ?>

                                </span>
                            </td>
                            <td class="px-8 py-6">
                                <?php if($request->price): ?>
                                    <p class="font-black text-primary"><?php echo e(number_format($request->price, 0, ',', ' ')); ?> <span class="text-[10px] opacity-50">CFA</span></p>
                                <?php else: ?>
                            ...
                            <td class="px-8 py-6">
                                <span class="px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full
                                    <?php echo e($request->status === 'pending' ? 'bg-amber-100 text-amber-700' : ''); ?>

                                    <?php echo e($request->status === 'processing' ? 'bg-blue-100 text-blue-700' : ''); ?>

                                    <?php echo e($request->status === 'quoted' ? 'bg-purple-100 text-purple-700' : ''); ?>

                                    <?php echo e($request->status === 'paid' ? 'bg-green-100 text-green-700' : ''); ?>

                                    <?php echo e($request->status === 'delivered' ? 'bg-indigo-100 text-indigo-700' : ''); ?>

                                    <?php echo e($request->status === 'cancelled' ? 'bg-red-100 text-red-700' : ''); ?>
                            ...
                            <td class="px-8 py-6 text-right">
                                <a href="<?php echo e(route('admin.services.show', $request)); ?>" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-primary hover:text-white transition-all text-xs">
    ...
    <div x-show="showTypeManager" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-[3rem] max-w-2xl w-full p-10 shadow-2xl overflow-hidden border border-slate-100">
                ...
                <div class="space-y-8">
                    <!-- Formulaire d'ajout -->
                    <form action="<?php echo e(route('admin.services.types.store')); ?>" method="POST" class="bg-slate-50 p-8 rounded-[2rem] space-y-4">
                        <?php echo csrf_field(); ?>
                        <div class="space-y-2">
                        ...
                        <?php $__currentLoopData = $serviceTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex items-center justify-between p-5 bg-white rounded-2xl border border-slate-100 hover:border-primary/20 transition-colors">
                                <div class="flex items-center gap-4">
                                    <div class="w-3 h-3 rounded-full <?php echo e($type->is_active ? 'bg-green-500 shadow-lg shadow-green-500/30' : 'bg-red-500 shadow-lg shadow-red-500/30'); ?>"></div>
                                    <span class="font-bold <?php echo e($type->is_active ? 'text-slate-900' : 'text-slate-300'); ?>"><?php echo e($type->name); ?></span>
                                </div>
                                <div class="flex items-center gap-3">
                                    <form action="<?php echo e(route('admin.services.types.toggle', $type)); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" class="text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl transition-all <?php echo e($type->is_active ? 'bg-amber-50 text-amber-600 hover:bg-amber-100' : 'bg-green-50 text-green-600 hover:bg-green-100'); ?>">
                                            <?php echo e($type->is_active ? 'Suspendre' : 'Activer'); ?>

                                        </button>
                                    </form>
                                    <form action="<?php echo e(route('admin.services.types.destroy', $type)); ?>" method="POST" onsubmit="return confirm('Supprimer définitivement ce service ?')">
                                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/monblog/resources/views/admin/services/index.blade.php ENDPATH**/ ?>
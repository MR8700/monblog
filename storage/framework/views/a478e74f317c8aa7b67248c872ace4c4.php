<?php $__env->startSection('title', 'Demande de Service - DigitalSpace'); ?>
...
<section class="max-w-4xl mx-auto px-6 py-12">
    <div class="flex flex-col gap-12">
        ...
        <form action="<?php echo e(route('services.request.store')); ?>" method="POST" enctype="multipart/form-data" class="space-y-8" 
            x-data="{ 
                fields: [],
                files: [],
                addFiles(event) {
            ...
            <div class="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/40 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="client_name" required value="<?php echo e(old('client_name')); ?>" class="w-full bg-slate-50 border-transparent rounded-2xl px-6 py-4 focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-medium">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">Email</label>
                        <input type="email" name="client_email" required value="<?php echo e(old('client_email')); ?>" class="w-full bg-slate-50 border-transparent rounded-2xl px-6 py-4 focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-medium">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">Téléphone / WhatsApp</label>
                        <input type="text" name="client_phone" required value="<?php echo e(old('client_phone')); ?>" class="w-full bg-slate-50 border-transparent rounded-2xl px-6 py-4 focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-medium">
                    </div>
                    <div class="space-y-2">
                        ...
                        <select name="service_type" required class="w-full bg-slate-50 border-transparent rounded-2xl px-6 py-4 focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-bold text-slate-700">
                            <?php $__currentLoopData = $serviceTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($type->name); ?>" <?php echo e(old('service_type') == $type->name ? 'selected' : ''); ?>><?php echo e($type->name); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <option value="Autre" <?php echo e(old('service_type') == 'Autre' ? 'selected' : ''); ?>>Autre (préciser dans la description)</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/40 space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="5" required class="w-full bg-slate-50 border-transparent rounded-3xl px-6 py-4 focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all font-medium" placeholder="Expliquez-nous votre besoin, vos contraintes, vos préférences..."><?php echo e(old('description')); ?></textarea>
                </div>
...
<?php echo $__env->make('layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/monblog/resources/views/public/services/request.blade.php ENDPATH**/ ?>
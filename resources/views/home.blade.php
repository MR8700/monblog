...
@section('title', 'Bienvenue sur Mon Espace Digital')
...
<section class="bg-white py-24 rounded-[5rem]">
  <div class="max-w-7xl mx-auto px-6">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
      ...
      <a href="{{ route('products.publicIndex') }}" class="group flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-slate-400 hover:text-primary transition-colors">
      ...
      @forelse ($products as $product)
        <div class="group bg-slate-50 rounded-[2.5rem] overflow-hidden border border-transparent hover:border-primary/10 hover:bg-white hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500">
          <div class="h-64 overflow-hidden relative">
            <img src="{{ $product->image ?? 'https://via.placeholder.com/600x400' }}" alt="{{ $product->title }}" 
                 class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
            <div class="absolute top-4 right-4 bg-white/90 backdrop-blur px-4 py-2 rounded-2xl shadow-sm">
              <span class="text-primary font-bold">{{ number_format($product->price, 0, ',', ' ') }} <span class="text-[10px]">CFA</span></span>
            </div>
          </div>
          <div class="p-8 space-y-4">
            ...
            <h3 class="text-xl font-bold text-slate-900">{{ $product->title }}</h3>
            <p class="text-sm text-slate-500 line-clamp-2 leading-relaxed">
              {{ $product->description }}
            </p>
            <a href="{{ route('products.show', $product) }}" class="inline-flex items-center gap-2 text-sm font-bold text-primary group-hover:gap-4 transition-all">
...
<section class="py-24">
  <div class="max-w-7xl mx-auto px-6">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 text-right md:text-left">
      ...
      <a href="{{ route('blog.index') }}" class="group flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-slate-400 hover:text-secondary transition-colors">
        ...
        <article class="group bg-white rounded-[2.5rem] p-8 border border-slate-100 hover:border-secondary/20 hover:shadow-2xl hover:shadow-secondary/5 transition-all duration-500 flex flex-col relative">
          ...
          <div class="space-y-4">
            ...
            <div class="pt-6 flex items-center justify-between">
              <a href="{{ route('blog.show', $post) }}" class="text-sm font-bold text-slate-900 group-hover:text-secondary flex items-center gap-2">
  ...
  <div class="bg-slate-900 rounded-[4rem] p-12 md:p-20 relative overflow-hidden group">
    ...
    <div class="relative z-10 grid md:grid-cols-2 gap-12 items-center">
      <div class="space-y-6">
        ...
        <div class="flex flex-wrap gap-4 pt-4">
          <a href="{{ route('chat.index') }}" class="px-10 py-5 bg-white text-slate-900 rounded-full font-bold hover:scale-105 transition-transform flex items-center gap-3">

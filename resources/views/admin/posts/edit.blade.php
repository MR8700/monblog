...
<div class="max-w-5xl mx-auto px-6 py-12">
  <div class="mb-8">
    <a href="{{ route('admin.posts.show', $post) }}" class="inline-flex items-center gap-2 text-primary hover:text-primary-dark transition">
      <i class="fas fa-chevron-left"></i> Retour
    </a>
  </div>

  <h1 class="text-3xl font-heading mb-8">Modifier l'article</h1>
      ...
      <div>
        ...
        @if($post->cover_image)
          <div class="mb-3 p-3 bg-slate-50 rounded-lg flex items-center gap-3">
            <img src="{{ asset('storage/' . $post->cover_image) }}" alt="Cover" class="w-16 h-16 rounded object-cover">
            <div class="flex-1">
              <p class="text-sm font-medium">Image actuelle</p>
              <p class="text-xs text-slate-500">Changez-la ci-dessous ou gardez l'actuelle</p>
            </div>
          </div>
        @endif
        <input 
          ...
          <div>
            ...
            >
              @foreach(\App\Enums\PostVisibility::cases() as $visibility)
                <option value="{{ $visibility->value }}" {{ old('visibility', $post->visibility->value) == $visibility->value ? 'selected' : '' }}>
                  {{ $visibility->label() }}
                </option>
              @endforeach
            </select>
